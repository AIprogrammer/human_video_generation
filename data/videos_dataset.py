...
class Videos_Dataset(BaseDataset):
        ...
        for folder in sample_folders:
            ...
            for i in range(0, opt.source_num):
                ...
                for k in range(1, 3):
                    ...
                    self.input_paths[-1]['previous_frame'].append(source_folders[i])
    ...
    def __getitem__(self, index):
        ...
        result_dict = {'input': [], 'target': [], 'previous_frame': [], 'grid': [], 'grid_source': []}
        output = {}
        output["paths"] = self.input_paths[index]
        for i in range(3):
            current_paths = self.input_paths[index]
            input_tensors = []
            img = Image.open(current_paths['texture'][i])
            ...
            img = Image.open(current_paths['previous_frame'][i])
            img_tensor = transform_img(img.convert('RGB'))
